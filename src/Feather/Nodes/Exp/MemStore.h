...
namespace Feather
{
    ...
    class MemStore : public DynNode
    {
        DEFINE_NODE(MemStore, nkFeatherExpMemStore, "Feather.Exp.MemStore");

    public:
        MemStore(const Location& loc, DynNode* value, DynNode* address, size_t alignment = 0, bool isVolatile = false, AtomicOrdering ordering = atomicNone, bool singleThreaded = false);

        /// Getter for the value to be stored
        DynNode* value() const;

        /// Getter for the node that represents the address where the value to be stored
        DynNode* address() const;
