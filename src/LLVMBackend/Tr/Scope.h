...
///
/// An object of this class adds itself automatically to the TrContext on constructor and removes
/// itself from TrContext in destructor. Therefore these are intended to be used as scoped objects
///
/// Beside the list of instructions, a scope has a list of destruct actions. The destruct actions
/// are called in reverse order at scope end (called from destructor), or when jumping outside the
/// scope (the one who makes the jump will call @a outputDestructActions() to write destruct
/// actions)
///
...
public:
    ...
    /// Note that at different points in the scope we might have zero instructions, or different
    /// instructions stacks
