...
{
    return genericParam(generic_->node(), idx);
}
...
ConversionType GenericCallable::canCall(CompilationContext* context, const Location& loc, const NodeVector& args, EvalMode evalMode, bool noCustomCvt)
{
    ...
    ASSERT(!inst_);
    inst_ = genericCanInstantiate(generic_->node(), argsWithCvt_);
...
Node* GenericCallable::generateCall(const Location& loc)
{
    ASSERT(inst_);
    ASSERT(context_);
    Node* res = genericDoInstantiate(generic_->node(), loc, context_, argsWithCvt_, inst_);
