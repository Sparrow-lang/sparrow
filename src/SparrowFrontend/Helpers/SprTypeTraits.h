...
namespace SprFrontend
{
    ...
    Nest::TypeRef doDereference1(DynNode* arg, DynNode*& cvt);
    ...
    Nest::TypeRef evalTypeIfPossible(DynNode* typeNode);
    
    /// create a type node from the given type
    DynNode* createTypeNode(CompilationContext* context, const Nest::Location& loc, Nest::TypeRef t);
    
    
    /// Gets the type to be used when auto is found for a node; removes l-values, but tries to preserve references
    Nest::TypeRef getAutoType(DynNode* typeNode, bool addRef = false);
