...
namespace SprFrontend
{
    ...
    Nest::TypeRef doDereference1(Nest::Node* arg, Nest::Node*& cvt);
    Nest::TypeRef doDereference1(Feather::DynNode* arg, Feather::DynNode*& cvt);
    ...
    Nest::TypeRef getType(Node* typeNode);
    Nest::TypeRef getType(Feather::DynNode* typeNode);
    
    /// Given a node representing a 'Type' value, get its type.
    /// Returns null if cannot evaluate node
    Nest::TypeRef tryGetTypeValue(Feather::DynNode* typeNode);
    Nest::TypeRef tryGetTypeValue(Nest::Node* typeNode);
    ...
    Nest::TypeRef evalTypeIfPossible(Feather::DynNode* typeNode);
    
    /// create a type node from the given type
    Feather::DynNode* createTypeNode(CompilationContext* context, const Nest::Location& loc, Nest::TypeRef t);
    
    
    /// Gets the type to be used when auto is found for a node; removes l-values, but tries to preserve references
    Nest::TypeRef getAutoType(Feather::DynNode* typeNode, bool addRef = false);
