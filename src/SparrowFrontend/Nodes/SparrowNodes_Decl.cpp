...
void handleStaticCtorDtor(Node* node, bool ctor) {
    ...
    Node* n = ctor ? Feather_mkGlobalConstructAction(node->location, funCall)
                   : Feather_mkGlobalDestructAction(node->location, funCall);
...
TypeRef SprDatatype_ComputeType(Node* node) {
    ...
    if (ifClause)
        REP_ERROR_RET(nullptr, node->location,
                "If clauses must be applied only to generics; this is not a generic class");
...
TypeRef SprParameter_ComputeType(Node* node) {
    ...
    Node* resultingParam = Feather_mkVar(
            node->location, Feather_getName(node), Feather_mkTypeNode(node->location, t));
...
void SprField_SetContextForChildren(Node* node) {
    ...
    // Create a new child compilation context if the mode has changed; otherwise stay in the same
    // context
...
TypeRef SprField_ComputeType(Node* node) {
    ...
    Node* resultingVar = Feather_mkVar(
            node->location, Feather_getName(node), Feather_mkTypeNode(node->location, t));
...
TypeRef SprVariable_ComputeType(Node* node) {
    ...
    if (varKind == varLocal) {
        ...
        expl = Feather_mkNodeList(
                node->location, fromIniList({resultingVar, ctorCall, dtorCall, nop}));
    } else {
