 ...
 *
 * @param ccloc         The compilation context & location of the caller
 ...
 */
ConversionType canCallGenericFun(CallableData& c, NodeRange args, EvalMode evalMode,
...
ConversionType CallableImpl::canCall(const CCLoc& ccloc, NodeRange args, EvalMode evalMode,
        CustomCvtMode customCvtMode, bool reportErrors) {
    NodeVector args2;
    ...
    if (data_.implicitArgType) {
        auto thisTempVar = Feather::VarDecl::create(
                ccloc.loc_, "tmp.v", Feather::TypeNode::create(ccloc.loc_, data_.implicitArgType));
    ...
    if (data_.type == CallableType::genericFun) {
        return canCallGenericFun(data_, args, evalMode, customCvtMode, reportErrors);
    }

    // Do the checks on types
    ConversionType res = canCall_common_types(
            data_, ccloc.context_, argTypes, evalMode, customCvtMode, reportErrors);
    if (!res)
        return convNone;
...
NodeHandle CallableImpl::generateCall(const CCLoc& ccloc) {
    ...
    if (data_.type == CallableType::function) {
        ...
        res = handleIntrinsic(FunctionDecl(data_.decl), ccloc.context_, ccloc.loc_, argsCvt);
        if (res) {
            res.setContext(ccloc.context_);
            return res;
        } else {
            // Otherwise, generate a function call
            res = createFunctionCall(ccloc.loc_, ccloc.context_, data_.decl, NodeRange(argsCvt));
        }
    }
    ...
    if (data_.type == CallableType::genericFun) {
        ASSERT(data_.genericInst);
        res = callGenericFun(GenericFunction(data_.decl), ccloc.loc_, ccloc.context_,
                NodeRange(data_.args), data_.genericInst);
        res.setContext(ccloc.context_);
    } else if (data_.type == CallableType::genericClass) {
        ASSERT(data_.genericInst);
        res = callGenericClass(GenericDatatype(data_.decl), ccloc.loc_, ccloc.context_,
                NodeRange(data_.args), data_.genericInst);
        res.setContext(ccloc.context_);
    } else if (data_.type == CallableType::genericPackage) {
        ASSERT(data_.genericInst);
        res = callGenericPackage(GenericPackage(data_.decl), ccloc.loc_, ccloc.context_,
                NodeRange(data_.args), data_.genericInst);
        res.setContext(ccloc.context_);
    }
    ...
    if (data_.type == CallableType::concept) {
        ...
        res = Feather_mkCtValueT(ccloc.loc_, StdDef::typeBool, &conceptFulfilled);
        res.setContext(ccloc.context_);
    ...
    if (data_.implicitArgType) {
        ...
        res = createTempVarConstruct(ccloc.loc_, ccloc.context_, res, data_.tmpVar);
    }
...
Callable* mkFunCallable(FunctionDecl fun, TypeWithStorage implicitArgType) {
