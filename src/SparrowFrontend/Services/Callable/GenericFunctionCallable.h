 ...
 * @see        Callable
 */
class GenericFunctionCallable : public Callable {
public:
    GenericFunctionCallable(GenericFunction decl, TypeWithStorage implicitArgType = {});
...
private:
    ...
    //! Create the final params that should appear in the instantiated function.
    //! This would add RT params, but also params corresponding to the concept params
    void createFinalParams(Nest::SmallVector<ParameterDecl>& finalParams);
    //! Create the instantiated datatype by cloning the original decl.
    SprFunctionDecl createInstantiatedFunction(NodeRangeT<ParameterDecl> finalParams);

    //! Get the final args that should be used when calling the generic function.
    //! This selects from the args the ones that correspond to final params.
    void getFinalArgs(Nest::SmallVector<NodeHandle>& finalArgs);
};
