...
/// A type that represents a generic set of types, types that model a given concept. If no concept
/// is given, the type will model any type
///
/// Alternatively, instead of a concept, we can pass a generic class; we use
/// the generic to behave like a concept
Nest_TypeRef getConceptType(
        Nest_Node* conceptOrGeneric = nullptr, uint8_t numReferences = 0, EvalMode mode = modeRt);

/// The concept associated with a ConceptType. Can be nullptr if no concept is used
/// Works only for concept types.
Nest_Node* conceptOfType(Nest_TypeRef type);
} // namespace SprFrontend
